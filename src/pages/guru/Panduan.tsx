...
export default function GuruPanduan() {
  const [pdfUrl, setPdfUrl] = useState('/panduan-guru.pdf')
  ...
  useEffect(() => {
    ...
    async function loadPdfUrl() {
      try {
        const response = await panduanAPI.get('guru')
        if (cancelled) return
      ...
      <div className="rounded-3xl bg-white p-7 shadow-sm ring-1 ring-slate-200">
        ...
        {(pdfStatus === 'available' || pdfStatus === 'unknown') && (
          <div className="mt-6 overflow-hidden rounded-2xl ring-1 ring-slate-200">
            <iframe title="Panduan Guru (PDF)" src={pdfUrl} className="h-[70vh] w-full" />
          </div>
        )}
